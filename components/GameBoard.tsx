...
const getTileVisuals = (name: string, type: 'building' | 'room' | 'street') => {
  ...
  if (n.includes('dock') || n.includes('river') || n.includes('pier') || n.includes('havn')) {
    return {
      bg: 'bg-[#172554]',
  ...
  return { bg: 'bg-[#262626]', style: {}, strokeColor: '#737373', Icon: Ghost, iconColor: 'text-neutral-500' };
};
      ...
      >
        {tiles.map(tile => {
          ...
          return (
            ...
            >
              <div className={`absolute inset-0 hex-clip transition-all duration-500 ${visual.bg} relative overflow-hidden`}>
                 {tile.imageUrl && <img src={tile.imageUrl} className="absolute inset-0 w-full h-full object-cover opacity-80 mix-blend-overlay animate-in fade-in duration-700" />}
                 {!tile.imageUrl && <div className="absolute inset-0 opacity-20" style={visual.style} />}
                 <div className="relative z-10 flex flex-col items-center opacity-40 pointer-events-none"><visual.Icon size={24} className={visual.iconColor} /></div>
                 
                 {tile.object && (
                     <div className="absolute bottom-4 z-20 animate-in zoom-in duration-300">
                         {tile.object.type === 'fire' && <Flame className="text-orange-500 animate-pulse" size={24} />}
                         {tile.object.type === 'locked_door' && <Lock className="text-amber-500" size={24} />}
                         {tile.object.type === 'rubble' && <Hammer className="text-stone-500" size={24} />}
                     </div>
                 )}
                 <div className={`absolute inset-0 transition-opacity duration-1000 bg-black z-30 pointer-events-none ${isVisible ? 'opacity-0' : 'opacity-80'}`} />
              </div>
              <svg viewBox="0 0 100 100" className="absolute inset-0 w-full h-full pointer-events-none z-10">
                  <polygon points={HEX_POLY_POINTS} fill="none" stroke={visual.strokeColor} strokeWidth="2.5" />
              </svg>
            </div>
          );
        })}
          ...
          return (
             ...
             >
                 <svg viewBox="0 0 100 100" className="absolute inset-0 w-full h-full overflow-visible">
                    <polygon points={HEX_POLY_POINTS} fill="rgba(255,255,255,0.05)" stroke="#e94560" strokeWidth="2" strokeDasharray="6,4" className="group-hover:stroke-white group-hover:stroke-[3px] transition-all" />
                 </svg>
             </div>
          );
        })}
